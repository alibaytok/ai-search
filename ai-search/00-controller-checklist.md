...
- [ ] For simple user-typed English word errors, try controlled-vocabulary spell correction before escalating to LLM, semantic modeling, benchmark design, ontology work, or broad FRAME-C rule changes. The correction vocabulary must come only from terms the downstream parser can actually consume; if correction is ambiguous, ask for clarification rather than forcing a single correction.
...
- [ ] Scope-boundary continuity is explicit: when a Work Order, review, triage, or closure packet ends and a distinct follow-up scope is next, prefer a handoff plus fresh chat for that next scope; use `/compact` only for same-scope continuation, small corrections, or status preservation inside the current scope. Do not force a handoff for ordinary questions, status checks, syntax fixes, clarification, or same-scope minor work.
- [ ] Forced handover is required before context transfer, after long-running work, after a halt, after review-time correction, and before any next Work Order prompt is issued; if the user explicitly chooses same-chat continuation, Claude must still provide the handoff-quality scope block before the next prompt.
